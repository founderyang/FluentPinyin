...
- Extract input mode transitions from `tsf_text_service.cpp`.
  Status: in progress. CapsLock and ASCII toggle transitions now live in
...
- Extract common string helpers into `common/encoding`.
  ...
  shortcut display trimming remain local.
- Extract installer cleanup primitives from `devtools/main.cpp`.
  Status: partially done. Reboot-deferred cleanup is removed from normal paths;
  larger command decomposition remains.
...
- Move duplicated Fluent SVG path constants into `common`.
  ...
  and candidate-page triangle paths now live in `src/common/svg_icons.h`, with
  TSF and settings keeping local aliases only for call-site stability.
...
Split `src/config_winui/main.cpp` by page and shared UI helpers:

- `settings_app.cpp`
- `pages/general_page.cpp`
- `pages/appearance_page.cpp`
- `pages/lexicon_page.cpp`
- `pages/hotkeys_page.cpp`
- `pages/sync_page.cpp`
