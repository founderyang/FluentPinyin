...
int wmain(int argc, wchar_t** argv) {
  ...
  if (command == L"cleanup-install" || command == L"cleanup-uninstall") {
    std::filesystem::path install_dir = fp::devtools::DefaultInstallDir();
    ...
    return fp::devtools::CleanupInstall(
        install_dir, skip_install_dir, keep_user_data, restart_text_services);
  }
