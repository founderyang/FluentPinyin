...
constexpr std::wstring_view kDefaultManagedUserLexiconName = L"fp_user_words";
...
std::vector<PhraseEntry> ParseTabSeparatedLexiconLines(std::wstring_view value) {
  ...
  while (std::getline(stream, line)) {
    ...
    PhraseEntry entry;
    entry.phrase = TrimText(columns[0]);
    entry.code = TrimText(columns[1]);
    if (columns.size() >= 3) {
...
bool WriteUserLexiconEntries(const std::vector<PhraseEntry>& entries) {
  ...
  for (const auto& entry : entries) {
    ...
    const std::wstring phrase = TrimText(entry.phrase);
    const std::wstring code = TrimText(entry.code);
...
bool ActivateExistingSettingsWindow(int attempts = 1) {
  const std::wstring title(kSettingsAppTitle);
  for (int attempt = 0; attempt < attempts; ++attempt) {
    const HWND hwnd = FindWindowW(nullptr, title.c_str());
    if (hwnd != nullptr) {
...
SizeInt32 DefaultWindowSize(Window const& window) {
...
void CenterWindowOnMonitor(Window const& window) {
  const HWND hwnd = GetWindowHandle(window);
  if (!hwnd) {
    return;
  }
    ...
    *add_row = [rows_panel, rows, update_empty_state](PhraseEntry entry) {
    ...
    operation.Completed([rows, user_lexicon, state_icon](auto const& async, auto const& status) {
      ...
      for (const auto& row : *rows) {
        ...
        PhraseEntry entry;
        entry.phrase = TrimText(row->phrase.Text().c_str());
        entry.code = TrimText(row->code.Text().c_str());
